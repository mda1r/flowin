    ...
    private async Task<string> BuildContextAsync(Guid branchId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (dailySummaries.Count > 0)
            {
                ...
                decimal monthRevenue = dailySummaries.Sum(s => s.TotalRevenue);
                int monthOrders = dailySummaries.Sum(s => s.TotalOrders);
                decimal monthTax = dailySummaries.Sum(s => s.TotalTax);
                decimal monthDiscounts = dailySummaries.Sum(s => s.TotalDiscounts);
                sections.Add($"ملخص الأسبوع:\n  الإيرادات: {monthRevenue:F2} | الطلبات: {monthOrders} | الضريبة: {monthTax:F2} | الخصومات: {monthDiscounts:F2} ر.س");
            }
        }
        catch { /* skip */ }

        // ── 2. Payment method breakdown (last 30 days) ───────────────────────
        try
        {
        ...
        // ── 3. Completed orders: peak hours + top products ────────────────────
        try
        {
            ...
            if (ordersWithLines.Count > 0)
            {
                ...
                sections.Add("أفضل ساعات البيع (آخر 30 يوم):\n" + string.Join("\n", peakLines));
        ...
        return sections.Count > 0
            ...
            : "لا توجد بيانات مبيعات مسجلة بعد في النظام.";
    }
}
