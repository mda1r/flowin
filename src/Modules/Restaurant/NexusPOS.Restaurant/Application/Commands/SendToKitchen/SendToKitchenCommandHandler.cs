...
internal sealed class SendToKitchenCommandHandler(
    IRestaurantOrderRepository orderRepository,
    RestaurantDbContext dbContext)
    : ICommandHandler<SendToKitchenCommand, RestaurantOrderResponse>
{
    public async Task<ErrorOr<RestaurantOrderResponse>> Handle(
        SendToKitchenCommand request,
        CancellationToken cancellationToken)
    {
        ...
        ErrorOr<Success> result = order.SendToKitchen();
        if (result.IsError)
        {
